...
def test_callback_links_to_existing_password_user(client, monkeypatch):
    ...
    _patch_oauth(monkeypatch, userinfo={"sub": "g-3", "email": "Shared@Gmail.com"})
    resp = client.get("/auth/google/callback?state=s&code=c", follow_redirects=False)
    assert resp.status_code == 303
